...
@pytest.fixture()
def client_with_plan_with_permanent_identifier_and_document(
    session: Session,
    client_with_plan_with_permanent_identifier: RyhtiClient,
    plan_instance: models.Plan,
    mock_public_map_document: Callable,
    ...
    authenticated to our mock X-Road API, and that has plan document uploaded.
    """
    responses = client_with_plan_with_permanent_identifier.upload_plan_documents()
    for plan_id, document_responses in responses.items():
        assert plan_id == plan_instance.id
...
@pytest.fixture()
def client_with_plan_with_permanent_identifier_and_document_in_proposal_phase(
    session: Session,
...
def test_upload_unchanged_plan_documents(
    ...
    reupload_responses = (
        client_with_plan_with_permanent_identifier_and_document.upload_plan_documents()
    )
    ...
    client_with_plan_with_permanent_identifier_and_document.set_plan_documents(
        reupload_responses
    )
    session.refresh(plan_instance.documents[0])
